...
/**
 * 0331 1.JtextField 의 데이터가 선택이안되도록 -->완료 0331 2.삭제 버튼클릭시 db에 데이터 삭제
 * 
 * @author user
 */
...
//		ArrayList<String> list = new ArrayList<String>();
		...
		titlePane2 = new JPanel();
		idPane2 = new JPanel();
		pwPane2 = new JPanel();
		namePane2 = new JPanel();
		majorPane2 = new JPanel();
		phonePane2 = new JPanel();
		genderPane2 = new JPanel();
		birthPane2 = new JPanel();
		jl_title = new JLabel("학생 정보 수정");
		jl_id = new JLabel("아이디");
		jl_pw = new JLabel("비밀번호");
		jl_name = new JLabel("이름");
		jl_major = new JLabel("학과");
		jl_phone = new JLabel("H.P");
		...
		bg_gender.add(female);
		bg_gender.add(male);

		// set font
		jl_title.setFont(AdmUI.FONTTITLE);
		jl_id.setFont(AdmUI.FONT);
		jl_pw.setFont(AdmUI.FONT);
		jl_name.setFont(AdmUI.FONT);
		jl_major.setFont(AdmUI.FONT);
		jl_phone.setFont(AdmUI.FONT);
		jl_gender.setFont(AdmUI.FONT);
		btn_delete.setFont(AdmUI.FONT);
		btn_cancel.setFont(AdmUI.FONT);
		male.setFont(AdmUI.FONT);
		female.setFont(AdmUI.FONT);
		jl_birth.setFont(AdmUI.FONT);
		// box레이아웃으로 수직정렬
...
//		list = {jl_title,jl_id,jl_pw,jl_name,jl_major,jl_phone,jl_gender};
		...
		namePane2.add(jt_name);
		majorPane.add(jl_major);
		majorPane2.add(jt_major);
		phonePane.add(jl_phone);
		phonePane2.add(jt_phone);
		birthPane.add(jl_birth);
		birthPane2.add(jt_birth);
		genderPane.add(jl_gender);
		genderPane2.add(female);
		genderPane2.add(male);
		buttonPane.add(btn_delete);
		buttonPane.add(btn_cancel);

		titlePane.add(jl_title);
		labelBoxPane.add(idPane);
		tfBoxPane.add(idPane2);
		labelBoxPane.add(pwPane);
		tfBoxPane.add(pwPane2);
		labelBoxPane.add(namePane);
		tfBoxPane.add(namePane2);
		labelBoxPane.add(majorPane);
		tfBoxPane.add(majorPane2);
		labelBoxPane.add(phonePane);
		tfBoxPane.add(phonePane2);
		labelBoxPane.add(birthPane);
		tfBoxPane.add(birthPane2);
		// 라디오옵션팬
...
//		add(buttonPane);
		...
		btn_delete.addActionListener(eventObj);
	}
...
//			StdVO svo = null;
//			int result = dao.getInfoListDel(svo);
			if (obj == btn_cancel) {
	...
	public void detailDel() {
		Vector<AdminStdVO> list = dao.getDetailSearch(detailData);
		String subNo = jt_id.getText().trim();
//		AdminStdVO svo = new AdminStdVO();
//		svo.setStdno(subNo);
//
//		for (AdminStdVO vo : list) {
//			if (vo.getStdno() == svo.getStdno()) {
//				svo.setStdno(vo.getStdno());
//			}
//		}

//		System.out.println(svo.getStdno());

		int result = dao.getInfoListDel(subNo);
//		System.out.println(svo.getStdno() + "삭제 확인");
		System.out.println(subNo + "삭제완료");

		dispose();

		if (result != 0) {
			int confirm = JOptionPane.showConfirmDialog(null, "정말삭제하시겠습니까?");
			if (confirm == 0) {
